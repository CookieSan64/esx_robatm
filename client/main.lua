...
AddEventHandler('esx_robatm:currentlyRobbing', function(currentStore)
...
RegisterNetEvent('esx_robatm:tooFar')
AddEventHandler('esx_robatm:tooFar', function()
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_cancelled'))
end)

RegisterNetEvent('esx_robatm:robberyComplete')
AddEventHandler('esx_robatm:robberyComplete', function(award)
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_complete', award))
end)

RegisterNetEvent('esx_robatm:startTimer')
AddEventHandler('esx_robatm:startTimer', function()
	...
	while true do
					...
					if distance < 0.5 then
						...
						if IsControlJustReleased(0, Keys['F']) then
							if IsPedArmed(PlayerPedId(), 4) then
								TriggerServerEvent('esx_robatm:robberyStarted', k)
							else
		...
		if holdingUp then
			...
			if Vdist(playerPos.x, playerPos.y, playerPos.z, storePos.x, storePos.y, storePos.z) > Config.MaxDistance then
				TriggerServerEvent('esx_robatm:tooFar', store)
			end
		end
	end
end)